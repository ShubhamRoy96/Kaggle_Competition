...
titanic_data = data %>% select(Pclass,Sex,SibSp,Parch,Survived)
test_data = test_data %>% select(Pclass,Sex,SibSp,Parch)
...
classifier = randomForest(x = titanic_data[-5], y = titanic_data$Survived, ntree = 100)
...
write.csv(result,"S:\\Learning\\ML\\Kaggle\\titanic\\titanic_predictions_Random_Forest_classification_Trimmed.csv",row.names = FALSE)
